...
engine specifically — including which fields are required, which are optional, which are
capability-gated, and what the engine rejects if they are absent.

The Run Request line-by-line reference follows the engine sections.
