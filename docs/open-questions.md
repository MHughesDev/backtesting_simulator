...
- **Open decisions (OD-1..OD-12)** = the blocking subset tracked in `SYS-001 §10`.
...
- **Q-STRAT-4** ~~How does a strategy express **pairs / spread / relative-value** logic across two instruments declaratively?~~ **RESOLVED:** cross-instrument references `data:<instrument>.field` / `signal:<instrument>.<id>` / `feature:<id>@<instrument>` ([strategy.md](specs/DATA-006-strategy-contract.md) §4).
...
- **Q-STRAT-7** ~~Can strategies be composed/nested (a strategy of strategies)?~~ **RESOLVED:** not nested — composed in a **Plan** by data-flow (screen→entry→exit), each strategy stays flat ([plan.md](specs/DATA-007-plan-contract.md), OD-6).
- **Q-STRAT-8** ~~How are strategy-level vs. portfolio-level constraints separated when multiple strategies share capital?~~ **RESOLVED:** Plan `account_mode` (shared/isolated) + `conflict_policy`; portfolio-level overlay stays the caller's analytics layer ([plan.md](specs/DATA-007-plan-contract.md)).
...
- **Q-STRAT-11** ~~Can a strategy define a derived/intermediate universe (e.g. "top-decile by a feature")?~~ **RESOLVED:** `universe.type: "scanner"` screens a cohort by point-in-time market-data + signal filters ([strategy.md](specs/DATA-006-strategy-contract.md) §6.3).
...
- **Q-RUNREQ-1** ~~Full **Run Request** schema~~ **RESOLVED:** defined in [run-request.md](specs/DATA-002-run-request.md) (OD-8).
...
- **Q-REG-1** ~~Trust model: Rust / PyO3 / WASM?~~ **RESOLVED:** tiered — built-in Rust / trusted native / **WASM sandbox** for untrusted/AI; Python excluded from the hot loop ([ADR-0011](adr/0011-component-registry-trust-model.md), [component-registry.md](specs/COMP-001-component-registry.md)).
