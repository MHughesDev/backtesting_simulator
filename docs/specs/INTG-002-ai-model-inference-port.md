# Spec: INTG-002 — AI Endpoint (Model Port)
...
`ts_available > current_ts` can appear in a bundle — described in [signals.md](DATA-005-signals-contract.md) §1)
and **determinism** (the same data + seed yields the same bundle, so inference is reproducible
provided the endpoint itself is seeded). Full mechanics: [signals.md](DATA-005-signals-contract.md) §5.
