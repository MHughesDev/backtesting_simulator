# Spec: DATA-003 — Instrument Contract
...
- **Each flag gates three things.** A flag simultaneously (1) declares which **payload variants**
  are valid for that instrument ([market-data.md](DATA-004-market-data-contract.md)), (2) permits or forbids certain
  **order types** (the order-type matrix in [engines/README.md](README.md)), and (3)
