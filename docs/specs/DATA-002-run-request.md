# Spec: DATA-002 — Run Request

**Spec ID:** DATA-002
**Type:** Data (per-invocation request schema)
**Status:** Approved
**Date:** 2026-06-06
**Author:** Agent
...
`issuer_id` (see [contracts/instrument.md](DATA-003-instrument-contract.md)); the engine resolves
...
`DerivedBar` event (see [contracts/market-data.md](DATA-004-market-data-contract.md) §2.26) into the
...
[contracts/signals.md](DATA-005-signals-contract.md); this is the binding surface.
...
Two non-negotiables for this plane (detail in [contracts/signals.md](DATA-005-signals-contract.md)):
...
A `scanner` universe ([contracts/strategy.md](DATA-006-strategy-contract.md) §6.3) surveys a **cohort**:
...
account, in [contracts/metrics.md](DATA-008-result-metrics-contract.md) (TBD) — not assumed by the core.
...
8. **Plan wiring** — if the `strategy` is a Plan, every `universe.from` names an existing strategy
   ...
   resolvable; else a typed `PlanWiringError` (see [contracts/plan.md](DATA-007-plan-contract.md) §8).
