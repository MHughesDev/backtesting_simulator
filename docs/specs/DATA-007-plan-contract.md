# Spec: DATA-007 — Plan Contract (multi-strategy composition)
...
- **Nested strategies** (a strategy contains a strategy contains a strategy) — turns the JSON into
  ...
  ([strategy.md](DATA-006-strategy-contract.md) §14.1): the format would become a programming language.
...
- **Concurrent independents:** strategies with no `from:` wiring simply run side by side. This is
  ...
  scanning.
- **No nesting:** `strategy` here is always a flat Strategy ([strategy.md](DATA-006-strategy-contract.md)); a Plan
...
At run start (extends [run-request.md](DATA-002-run-request.md) §10):
...
- Each contained Strategy validates independently ([strategy.md](DATA-006-strategy-contract.md)).
