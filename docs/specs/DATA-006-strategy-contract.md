# Spec: DATA-006 — Strategy Contract
...
- **Extensibility is via the component registry, not via free-form code.** When a built-in
  ...
  [component-registry.md](COMP-001-component-registry.md).**
...
| | **Strategy JSON** | **Run Request** ([run-request.md](DATA-002-run-request.md)) |
|---|---|---|
...
| `"signal:news_volume"` | an exogenous signal stream on the current instrument (see [signals.md](DATA-005-signals-contract.md)) |
...
is gated by `UniverseMembership` (see [market-data.md](DATA-004-market-data-contract.md) §2.25):
...
- Instruments are **materialized from the cohort data source** as they appear (see
  [run-request.md](DATA-002-run-request.md) §4d); they need not be enumerated in advance.
...
- A scanner **selects candidates; it does not by itself decide entries.** Admission produces a
  ...
  separate **entry strategy** wired in a **Plan** (see [plan.md](DATA-007-plan-contract.md)). This is what prevents
...
block, see [run-request.md](DATA-002-run-request.md) §4c) and a `lookback` window:
...
compile time. See [engines/README.md](README.md) order-type matrix.
...
the degenerate one-node Plan. Full spec: [plan.md](DATA-007-plan-contract.md).
...
> **Resolved since first draft:** multi-strategy portfolios → the **Plan** layer ([plan.md](DATA-007-plan-contract.md),
> OD-6); component-registry trust model → tiered built-in/native/WASM ([component-registry.md](COMP-001-component-registry.md),
> ADR-0011); Run Request schema → [run-request.md](DATA-002-run-request.md); dynamic-universe
> survivorship → point-in-time `UniverseMembership` ([market-data.md](DATA-004-market-data-contract.md) §2.25).
