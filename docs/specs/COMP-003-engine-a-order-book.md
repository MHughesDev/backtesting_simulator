# Spec: COMP-003 — Engine A: Order Book (CLOB)
...
instrument (see [contracts/market-data.md](DATA-004-market-data-contract.md)).
...
- **Heuristic** (L1/bar): `slippage_bps = impact(order_notional / interval_volume)` via a
  ...
  calibration; the conservative tables in [assets/crypto-spot-cex.md](DATA-011-crypto-spot-cex-asset-spec.md)
...
(see [assets/perpetuals.md](DATA-014-perpetuals-asset-spec.md) §2.5).

---

## 7. Output

Each fill (or rejection) produces a `TradeRecord` (see [run-request.md](DATA-002-run-request.md) §7):
