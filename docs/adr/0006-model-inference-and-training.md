...
rolling window). The suite must support inference cleanly, allow opt-in training without
becoming a training framework, and keep both leak-free and reproducible.

## Decision

Models are an **external dependency invoked through the Model Contract**; the suite stores no
weights. **Inference is the default and only behavior unless a strategy explicitly opts in to
training.** When opted in, the suite **orchestrates** training (decides *when* to refit and
assembles strictly point-in-time `ts_event ≤ current_ts` training data) but the **training
method is a registered, preconfigured routine owned by the caller** — the suite does not own
training algorithms.
...
- **Suite owns a training framework** — powerful, but turns the backtester into an ML platform,
...
- **Positive:** clean inference path; walk-forward and scenario fitting available without the
  suite owning ML; leak-free by construction (PIT-enforced); reproducible via version pinning +
  seeds + refit caching.
